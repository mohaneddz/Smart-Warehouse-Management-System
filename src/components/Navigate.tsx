...
function CarouselSpacing({ onCardClick }: CarouselSpacingProps) {
  ...
  return (
    <div className="w-full max-w-md mx-auto  dark text-xl sm:text-[12px] md:text-xs lg:text-xl">
      <div className="relative grid grid-cols-3 place-items-center gap-8 justify-items-center-center py-8">
        {cards.map((cardLabel, index) => (
          <Card
            key={cardLabel}
            onClick={() => handleCardClick(index)}
            className={`flex justify-center items-center  border-[#8B939B]
              absolute transition-all duration-300 ease-in-out cursor-pointer w-[45%] h-10  bg-[#2C2F3C]
              ${index === 0 ? 'left-0 -translate-x-1/2' : ''}
              ${index === 1 ? 'z-40 scale-120 bg-[#13101E] shadow-[0_0_15px_#6B86DE3B] border-[#8B939B] opacity-100' : 'z-10 scale-90 opacity-70 hover:opacity-90'}
